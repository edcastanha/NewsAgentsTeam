...
class NewsProcessor(MessageProcessor):
    ...
    def normalize_text(self, text):
        ...
        logger.debug(filtered_words)
        return " ".join(filtered_words)

    def publish_classification_task(self, source_id, title_normalized, content_normalized):
        ...
        if success:
            logger.info(f"Mensagem para classificação publicada para Source ID: {source_id}")
            return True
        else:
            logger.error(f"Erro ao publicar mensagem para classificação para Source ID: {source_id}")
            return False
